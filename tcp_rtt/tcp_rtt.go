...
func (s *statsStream) Reassembled(reassemblies []tcpassembly.Reassembly) {
	//DO WE NEED TO USE THE TCPREADHER???!?!
	for _, reassembly := range reassemblies {
		...
		//log.Printf("Reassembly going one for packet with %v Bytes", int64(len(reassembly.Bytes)))
		var tcp layers.TCP
		var df gopacket.DecodeFeedback
		//update mean rtt we must look at data

		if int64(len(reassembly.Bytes)) > 20 {
			err := tcp.DecodeFromBytes(reassembly.Bytes, df) //protect from error.
			if err != nil {
				continue
			}
			...
			for i := range tcp.Options {
				if tcp.Options[i].OptionType == 8 {
					...
					//tsecr := binary.BigEndian.Uint64(tcp.Options[i].OptionData[4:])
					if s.packets == 1 {
						s.last_ts = ts
					} else if s.packets == 2 {
						s.rtt = int64(ts - s.last_ts)
						s.last_ts = ts
					} else {
						s.rtt *= (s.packets - 2)
						s.rtt += int64(ts - s.last_ts)
...
func (s *statsStream) ReassemblyComplete() {
	...
	log.Printf("Reassembly of stream %v:%v complete - start:%v end:%v bytes:%v packets:%v ooo:%v bps:%v pps:%v skipped:%v rtt:%v",
		s.net, s.transport, s.start, s.end, s.bytes, s.packets, s.outOfOrder,
		float64(s.bytes)/diffSecs, float64(s.packets)/diffSecs, s.skipped, s.rtt)
}
		...
		for _, typ := range decoded {
			...
			case layers.LayerTypeTCP:
				if foundNetLayer {
					...
					assembler.AssembleWithTSnHeaders(netFlow, &tcp, ci.Timestamp)
				} else {
